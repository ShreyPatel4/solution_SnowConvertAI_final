...
*/
    -- Computed persisted
    -- EVEN, SEASONAL, CUSTOM, PRIOR_YEAR
    -- GUID type
    -- HASHBYTES computed
CREATE OR REPLACE TABLE Planning.BudgetLineItem (
    BudgetLineItemID BIGINT IDENTITY(1,1) ORDER NOT NULL,
    ...
    FinalAmount VARIANT AS (OriginalAmount + AdjustedAmount) /*** SSC-FDM-TS0014 - COMPUTED COLUMN WAS TRANSFORMED TO ITS SNOWFLAKE EQUIVALENT, FUNCTIONAL EQUIVALENCE VERIFICATION PENDING. ***/,
    ...
    LastModifiedDateTime TIMESTAMP_NTZ(7) NOT NULL DEFAULT SYSDATE(),
    RowHash VARBINARY AS SHA2(CONCAT(CAST(GLAccountID AS VARCHAR), '|',
                                         CAST(CostCenterID AS VARCHAR), '|',
                                         CAST(FiscalPeriodID AS VARCHAR), '|',
                                         CAST(/*FinalAmount*/OriginalAmount + AdjustedAmount AS VARCHAR)), 256) /*** SSC-FDM-TS0014 - COMPUTED COLUMN WAS TRANSFORMED TO ITS SNOWFLAKE EQUIVALENT, FUNCTIONAL EQUIVALENCE VERIFICATION PENDING. ***/,

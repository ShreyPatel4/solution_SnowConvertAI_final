-- ==============================================================================
-- SQL Server baseline proc 3: loads original usp_ExecuteCostAllocation as-is
-- (with surgical patches flagged inline).
-- Content below is the verbatim source from
--   original/src/StoredProcedures/usp_ExecuteCostAllocation.sql,
-- preceded by the SET/USE/DROP preamble needed to make the file re-loadable.
--
-- Patches applied to the original (each marked with a "PATCH:" comment):
--   1. The UPDATE-with-OUTPUT-INTO clause targeting #AllocationResults was
--      removing 4 column values into a table that declares 7 NOT-NULL columns
--      (TargetCostCenterID, TargetGLAccountID, IterationNumber are omitted).
--      SQL Server fails this with a NOT NULL violation at runtime.  It is
--      also semantically wrong: the source gross-amount was being captured
--      into AllocatedAmount, and RemainingAmount into AllocationPercentage.
--      The subsequent explicit INSERT INTO #AllocationResults does the real
--      work.  Patch: drop the OUTPUT .. INTO clause; keep the UPDATE.
-- ==============================================================================
...
USE Planning;
GO
IF OBJECT_ID('Planning.usp_ExecuteCostAllocation', 'P') IS NOT NULL
    DROP PROCEDURE Planning.usp_ExecuteCostAllocation;
GO
...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            -- PATCH: original had OUTPUT deleted.{SourceBudgetLineItemID,
            -- AllocationRuleID, SourceAmount, RemainingAmount} INTO
            -- #AllocationResults (SourceBudgetLineItemID, AllocationRuleID,
            -- AllocatedAmount, AllocationPercentage).  This is broken two ways:
            --   (a) #AllocationResults has 3 other NOT-NULL columns
            --       (TargetCostCenterID, TargetGLAccountID, IterationNumber)
            --       which the OUTPUT list doesn't supply -> runtime NOT NULL
            --       violation.
            --   (b) It mis-assigns the SourceAmount (gross) into the
            --       AllocatedAmount column and RemainingAmount into the
            --       AllocationPercentage column.
            -- The subsequent explicit INSERT INTO #AllocationResults block
            -- does the real computation.  Dropping the OUTPUT INTO clause is
            -- behaviour-preserving (downstream read uses ProcessedDateTime).

-- ==============================================================================
-- SQL Server baseline proc 1: loads original usp_ProcessBudgetConsolidation as-is
-- Content below is a verbatim copy of original/src/StoredProcedures/
--   usp_ProcessBudgetConsolidation.sql, preceded only by the SET/USE/DROP
--   preamble needed to make the file re-loadable.
-- ==============================================================================
...
USE Planning;
GO
IF OBJECT_ID('Planning.usp_ProcessBudgetConsolidation', 'P') IS NOT NULL
    DROP PROCEDURE Planning.usp_ProcessBudgetConsolidation;
GO
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- PATCH: The original block built dynamic SQL that referenced
        -- @ConsolidatedAmounts (a table variable) via sp_executesql.  That
        -- fails with "Must declare the table variable" — dynamic SQL runs
        -- in a separate batch and cannot see the caller's table variables.
        -- Fix: inlined the UPDATE directly (also removes the REPLACE()-based
        -- option rewriting in favour of structured IF branches, matching the
        -- Snowflake migration's approach).
        -- =====================================================================
        ...
        BEGIN
            ...
            DECLARE @IncludeZeroBalances BIT = 1;
            DECLARE @RoundingPrecision INT;

            IF @ProcessingOptions IS NOT NULL
            BEGIN
                SELECT
                    @IncludeZeroBalances = ISNULL(@ProcessingOptions.value('(/Options/IncludeZeroBalances)[1]', 'BIT'), 1),
                    @RoundingPrecision   = @ProcessingOptions.value('(/Options/RoundingPrecision)[1]', 'INT');
            END

            IF @RoundingPrecision IS NULL
            BEGIN
                UPDATE ca
                SET FinalAmount = ca.ConsolidatedAmount - ca.EliminationAmount
                FROM @ConsolidatedAmounts ca
                WHERE (ca.ConsolidatedAmount <> 0 OR ca.EliminationAmount <> 0)
                  AND (@IncludeZeroBalances = 1 OR (ca.ConsolidatedAmount - ca.EliminationAmount) <> 0);
            END
            ELSE
            BEGIN
                UPDATE ca
                SET FinalAmount = ROUND(ca.ConsolidatedAmount - ca.EliminationAmount, @RoundingPrecision)
                FROM @ConsolidatedAmounts ca
                WHERE (ca.ConsolidatedAmount <> 0 OR ca.EliminationAmount <> 0)
                  AND (@IncludeZeroBalances = 1 OR ROUND(ca.ConsolidatedAmount - ca.EliminationAmount, @RoundingPrecision) <> 0);
            END

            SET @AllocationRowCount = @@ROWCOUNT;
        ...
        SELECT 
            ...
            ca.FinalAmount,
            0,
            'CONSOL',  -- PATCH: original wrote 'CONSOLIDATED' (12 chars) into SpreadMethodCode VARCHAR(10) — truncation error
            'CONSOLIDATION_PROC',

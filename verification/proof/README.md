...
| `04_proc2_smoke.log` | Proc 2 hybrid-AI smoke-test `CALL`. Return object: `status=COMPLETED`, `exit_code=0`, 3 steps completed, 0 failed. |
| `05_proc4_smoke.log` | Proc 4 hybrid-AI smoke-test `CALL`. Return object: `success=true`, 18 historical rows, forecast accuracy metrics populated. |
| `06_proc5_smoke.log` | Proc 5 hybrid-AI smoke-test `CALL`. Return object: `success=true`, `unreconciled_count=0`, full reconciliation report. |
| `07_proc6_smoke.log` | Proc 6 hybrid-AI smoke-test `CALL`. Return object: `success=true`, `rows_imported=1`, `invalid_rows=0`. |
